...
const firstImportPreview = pendingImportPreview;
const importDialog = page.locator('[role="dialog"]');
await importDialog.waitFor({ state: 'visible' });
if (!(await importDialog.textContent())?.includes('正在导入')) throw new Error('订阅预览未显示加载状态');
await page.keyboard.press('Escape');
await page.locator('[role="dialog"]').waitFor({ state: 'hidden' });
if (await importButton.isDisabled()) throw new Error('取消预览后导入按钮仍处于忙碌状态');
const firstImportPreviewResponse = page.waitForResponse((response) => new URL(response.url()).pathname === '/api/import/preview');
firstImportPreview.release();
await firstImportPreviewResponse;
await page.waitForTimeout(100);
if (await page.locator('[role="dialog"]').count()) throw new Error('已取消的订阅预览响应重新打开了对话框');

const secondImportPreviewRequest = page.waitForRequest((request) => new URL(request.url()).pathname === '/api/import/preview');
await importButton.click();
await secondImportPreviewRequest;
