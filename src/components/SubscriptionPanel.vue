...
import type { SubscriptionsData, SubRefreshData, SubRefreshItem, ImportData } from '@/api/types';

const toast = useToast();

const subNames = ref<string[]>([]);
const refreshing = ref(false);
const refreshResults = ref<SubRefreshItem[] | null>(null);
...
async function doImport() {
  ...
  if (
    !confirm(
      `确认${what}？\n\nURL: ${url}\n\n将写入 ~/.config/mihomo/config.yaml（自动备份 .bak.*，失败自动回滚），随后热加载。`
    )
  ) {
    return;
  }
  ...
  try {
    const d = await api<ImportData>('/import', {
      method: 'POST',
      body: { url, provider: name, reload: impReload.value },
      timeout: 120000,
    });
    impError.value = '';
...
<template>
  <div class="card">
    <h2>订阅管理</h2>
    <div class="row">
      <button class="primary" :disabled="refreshing" @click="refreshSubs">刷新全部订阅</button>
      <span class="muted">{{ subNames.length ? '订阅源: ' + subNames.join(', ') : '未配置订阅源' }}</span>
    </div>
    <div class="sub-result">
      <span v-if="refreshing" class="muted">刷新中（热加载 + 验证缓存文件变化）…</span>
      ...
      <span v-else-if="refreshError" class="fail">{{ refreshError }}</span>
    </div>
    <hr class="sep" />
    <div class="sub-title">导入订阅源（写入 config.yaml，自动备份 + 热加载）</div>
    <div class="form-row">
    ...
    <div class="row import-row">
      <button :disabled="impBusy" @click="doImport">导入并热加载</button>
      <label class="chk">
